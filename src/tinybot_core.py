...
from utils import load_config, setup_logger, log_message, log_debug, discover_agents, discover_skills
...
class TinyBotCore:
        ...
        try:
            if ":" in class_path:
                ...
                agent_instance = agent_class(
                    self.config, agent_def, self.global_llm_caller_func, 
                    self.session_manager.state["log_file"], available_skills=agent_skills
                )
    ...
    def process_user_input(self, user_input):
        ...
        if user_input.strip().lower() == '/exit':
            ...
            if active_agent:
                ...
                if not memory_path:
                    ...
                    memory_path = os.path.join(tinybot_root, "agents", active_agent_key.lower().replace("agent", ""), "memory.md")
                ...
                response_content = active_agent.last_response if hasattr(active_agent, 'last_response') else "Exiting... *beep*"
        ...
        # Capture Response
        response_content = active_agent.last_response if hasattr(active_agent, 'last_response') and active_agent.last_response else "No direct response from agent."
        ...
        if transition_signal:
            ...
            if target_agent_key in state["agents"]:
                ...
                if hasattr(new_active_agent, 'last_response') and new_active_agent.last_response:
                    response_content += f"\n\n{new_active_agent.last_response}"
            else:
                 log_debug(f"Warning: Agent returned unknown transition signal: '{transition_signal}'")
    ...
    def handle_orchestrator_command(self, user_input):
        ...
        if command == '/debug':
            ...
            response_messages.append("Session summarization initiated... *whir* (Feature pending implementation in core)")
        elif command == '/help':
            response_messages.append("Available Commands: /debug, /agents, /summarize, /help, /exit")
        else:
